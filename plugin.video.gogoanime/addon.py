...
# def pagination():
#     print('<Pagination> Plugin path is [' + plugin.path + ']')
...
#         else:
#             for a in document.find_all('a'):
#                 print('<Pagination> Item label is [' + u'[{}] {} {}'.format(a.find('span', {'class': 'type'}).text, a.find('h3').text, a.find('span', {'class': 'ep'}).text) + ']' )
...
#                 item.setInfo('video', {})
#                 item.setProperty('IsPlayable', 'true')
#                 print('<Pagination> Item path is [' + a.attrs['href'] + ']' )
...
#                 print('<Pagination> Next path is [' + plugin.path + li.find('a').attrs['href'] + ']')
...
# @plugin.route('/[^/]+.html')
# def play_episode():
#     print('<play_episode> Plugin path is [' + plugin.path + ']')
#     response = request(plugin.path)
#     document = BeautifulSoup(response.text, 'html.parser')
#     title = document.find('h1').text.strip()
#     print('<play_episode> title is [' + title + ']')
...
#             print('<play_episode> Resolved url is [' + url + ']')
